  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "14px 18px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontWeight: 700, fontSize: 14, color: "#0f172a", fontFamily: "'Sora',sans-serif" }}>
            ...
            <div key={item.movil_id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 18px", borderBottom: "1px solid #f8fafc" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ margin: 0, fontSize: 11, color: "#94a3b8" }}>
                  {item.almacenamiento}GB · x{item.cantidad}
                </p>
              </div>
              {/* Precio + eliminar */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                ...
                <button
                  onClick={() => handleRemove(item.movil_id)}
      ...
      {items.length > 0 && (
        <div style={{ padding: "14px 18px", borderTop: "1px solid #f1f5f9", background: "#fafafa" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 16, fontWeight: 800, color: "#4f46e5", fontFamily: "'Sora',sans-serif" }}>{fmt(total)}</span>
          </div>
          <button
            onClick={() => { onClose(); navigate("/carrito") }}
            style={{ width: "100%", padding: "11px", background: "linear-gradient(135deg,#6366f1,#4f46e5)", color: "#fff", border: "none", borderRadius: 10, fontSize: 13.5, fontWeight: 700, cursor: "pointer", fontFamily: "'Sora',sans-serif", boxShadow: "0 4px 12px rgba(99,102,241,.35)" }}
          >
            Ir al carrito →
          </button>
        </div>
  ...
  if (!isAuthenticated) {
    buttons = (
      <div className="flex gap-3">
        <button onClick={() => navigate("/login")} className="text-sm font-medium hover:underline">
          Iniciar sessió
        </button>
        <button onClick={() => navigate("/register")} className="bg-black text-white px-4 py-2 rounded text-sm">
          Registre
        </button>
      </div>
    )
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <span className="absolute inset-y-0 right-2 grid w-8 place-content-center">
                  <button type="button" onClick={handleSearch} aria-label="Buscar" className="rounded-full p-1.5 text-gray-600 hover:bg-gray-100">
                    🔍
                  </button>
                </span>
              </div>
            ...
            <div className="flex items-center gap-4 shrink-0">
              ...
              <div ref={cartRef} style={{ position: "relative" }}>
                <button
                  ...
                  style={{
                    position: "relative", background: cartOpen ? "#f1f5f9" : "none",
                    border: "1px solid #e2e8f0", borderRadius: 10,
                    padding: "8px 10px", cursor: "pointer",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    transition: "background .15s",
                  }}
                  onMouseEnter={(e) => { if (!cartOpen) e.currentTarget.style.background = "#f8fafc" }}
                  onMouseLeave={(e) => { if (!cartOpen) e.currentTarget.style.background = "none" }}
                >
