...
function generarFacturaHTML(compraId, arts, total, usuario, etiquetas, localeFecha) {
  ...
  const clienteNombre = c.nombre && c.apellidos
      ...
      <button onClick={alAlternar} className="tarjeta-pedido-btn">
        <div className="tarjeta-pedido-izq">
          ...
          <div className="pedido-meta">
            ...
            <p className="pedido-resumen">
              {t('orders.productCount')(totalProductos)} · {pedido.metodo_pago}
            </p>
          </div>
        </div>
        ...
        <div className="tarjeta-pedido-body">
          ...
          {pedido.items?.map((art, i) => (
            <ElementoPedido
              key={i}
              art={art}
              movil={cachMoviles[art.movil_id]}
              t={t}
            />
          ))}
          ...
          <div className="pedido-factura-wrap">
            <button onClick={verFactura} className="btn-factura">
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        #pedidos-cab { margin-bottom: 28px; }
        #pedidos-titulo {
          margin: 0;
          font-size: 26px;
          font-weight: 800;
        ...
        .pedidos-vacio-icono { font-size: 48px; margin-bottom: 12px; }
        .pedidos-vacio-titulo {
          margin: 0;
          font-size: 16px;
          font-weight: 700;
          color: #0f172a;
      ...
      <Navbar />
      <div id="pag-pedidos">
        <div id="pedidos-contenedor">

          <div id="pedidos-cab">
            <h2 id="pedidos-titulo">{t('orders.title')}</h2>
            {!cargando && (
              <p id="pedidos-conteo">{t('orders.count')(pedidos.length)}</p>
            )}
          </div>

          {cargando ? (
            <div id="pedidos-spinner">
              <svg id="spinner-pedidos" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2.5">
                <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" />
              </svg>
            </div>
          ) : pedidos.length === 0 ? (
            <div id="pedidos-vacio">
              <div className="pedidos-vacio-icono">📦</div>
              <p className="pedidos-vacio-titulo">{t('orders.empty')}</p>
              <p className="pedidos-vacio-desc">{t('orders.emptyDesc')}</p>
              <button className="btn-ir-tienda" onClick={() => window.location.href = "/"}>
                {t('orders.goToShop')}
              </button>
            </div>
          ) : (
